...
namespace GestionImpresoras.Controllers
{
    public class ImpresorasController : Controller
    {
        ...
        public async Task<IActionResult> Crear()
        {
            ...
            if (!await _contexto.Modelos.AnyAsync())
            {
                ModelState.AddModelError(string.Empty, "La tabla de modelos está vacía");
                return View();
            }
            if (!await _contexto.Areas.AnyAsync())
            {
                ModelState.AddModelError(string.Empty, "La tabla de áreas está vacía");
                return View();
            }
            if (!await _contexto.Unidades.AnyAsync())
            {
                ModelState.AddModelError(string.Empty, "La tabla de unidades está vacía");
                return View();
            }
            if (!await _contexto.Estados.AnyAsync())
            {
                ModelState.AddModelError(string.Empty, "La tabla de estados está vacía");
                return View();
            }
            if (!await _contexto.Instituciones.AnyAsync())
            {
