...
namespace GestionImpresoras.Controllers
{
    public class SolicitudesController : Controller
    {
        ...
        // Acción para procesar el cambio de estado
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> ChangeStatus(int id, [Bind("Id,EstadoSolicitudId")] Solicitud solicitud)
        {
            if (id != solicitud.Id)
            {
                return NotFound();
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    var existingSolicitud = await _contexto.Solicitudes.FindAsync(id);

                    if (existingSolicitud != null)
                    {
                        existingSolicitud.EstadoSolicitudId = solicitud.EstadoSolicitudId;
                        _contexto.Update(existingSolicitud);
                        await _contexto.SaveChangesAsync();
                    }
                    else
                    {
        ...
        {
            return _contexto.Solicitudes.Any(e => e.Id == id);
        }
